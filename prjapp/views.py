...
from .decorators import unauth_user,allowed_users,auth_user , admins
...
def verify_email(request):
    ...
    customer.save()
    #email_sender(request , code)
    html_message = render_to_string('store_temps/email.html' , {'code':code})
    plan_message = strip_tags(html_message)

    message = EmailMultiAlternatives(
        subject= "GoldeCodes Verification",
        body= plan_message,
        from_email= None , 
        to= [customer]
    )
    ...
    #send_mail("GoldeCodes Verification " , message.attach_alternative(html_message , "text/html") , settings.EMAIL_HOST_USER , [customer] , fail_silently=False)
...
def email_sender(request):
    ...
    context ={'code':code}
    html_content = render_to_string('store_temps/email.html' ,{'code':code})
    return HttpResponse(html_content)
    # return render(request , 'store_temps/email.html' , context)
...
def account(request):
    ...
    if customer.verified == False:
        messages.success(request , ("Please Verified Your Account First"))
        return redirect('verifyemail')
    ...
    for i in orders:
        sort_id.append(i.id)
    sort_orders_id = sorted(sort_id, reverse=True)
    sort_order = []
    for i in sort_orders_id:
        for j in orders:
            if i == j.id:
                sort_order.append(j)

    context = {'sort_order':sort_order,'k':k,'products':products , 'orders':orders ,'items':items ,'total_orders':total_orders}
...
#         pass
    ...
    context={'randon':randon,'customer':customer,'fad':fad,'ad':ad,'trending':trending,'what':what,'tele':tele,'is_admin':bool,'form':form,'products':products , 'cartItems':cartItems }
...
def get_transaction_info(request):
    ships= Shippingadd.objects.all()
    return JsonResponse({"ships":list(ships.values())})
...
def processOrder(request):
    ...
    total2 = float(data['total2'])
    total = float(data['form']['total'])
    order.transaction_id = transaction_id
    if total == order.get_cart_total or total == total2:
        order.complete = True
...
def verifed_coupon(request):
    data = json.loads(request.body)
    couponn = data['couponn']
    try:
        cou = Coupons.objects.get(coupon = couponn )
        cou.delete()
    except:
        pass
    return JsonResponse('payment complete',safe=False)
...
def FreeFirePubg(request):
    ...
    cartItems = data['cartitems']
    context = {'cartItems':cartItems , 'customer':customer}
    return render (request , 'search_page/FreeFire&Pubg.html' ,context)
...
def Nintendo(request):
    ...
    cartItems = data['cartitems']
    context = {'cartItems':cartItems , 'customer':customer}
    return render (request , 'search_page/Nintendo.html' ,context)
...
def Amazon_uk(request):
    ...
    cartItems = data['cartitems']
    context = {'cartItems':cartItems , 'customer':customer} 
    return render(request , 'search_page/Amazon_uk.html',context)
##############################
@auth_user
def Amazon_german(request):
    cust = request.user.customer
    customer = Customer.objects.get(email = cust)
    if customer.suspand == True:
        logout(request)
    ...
    cartItems = data['cartitems']
    context = {'cartItems':cartItems , 'customer':customer}
    return render(request , 'search_page/Amazon_german.html',context)
##############################
...
def Razer_Gold_Global(request):
    ...
    cartItems = data['cartitems']
    context = {'cartItems':cartItems , 'customer':customer}
    return render(request , 'search_page/Razer_Gold_Global.html',context)
##############################
@auth_user
def Uber(request):
    cust = request.user.customer
